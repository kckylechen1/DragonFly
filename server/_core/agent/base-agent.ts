...
export abstract class BaseAgent {
  ...
  async *stream(userMessage: string): AsyncGenerator<StreamEvent> {
    ...
    yield {
      type: "thinking",
      data: `分析问题 (${this.state.queryComplexity})...`,
    };
  ...
  private async handleToolCalls(response: LLMResponse): Promise<void> {
    this.log(
      `🔧 工具调用: ${response.tool_calls!.map(tc => tc.function.name).join(", ")}`
    );
   ...
   * 执行单个工具（带指数退避重试）
   */
  private async executeSingleToolWithRetry(
    toolCall: ToolCall,
    maxRetries = 3
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        this.log(
          `   ⚠️ 工具 ${toolCall.function.name} 第 ${attempt + 1} 次失败，${delay / 1000}s 后重试...`
        );
