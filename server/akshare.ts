 ...
 */
export async function callAKShare<T = any>(
  endpoint: string,
  ...
  if (!isAvailable) {
    throw new Error(
      `AKTools 服务未运行。请执行 "pnpm start:aktools" 启动服务。`
    );
  }
...
export async function getLongHuBangDaily(date: string): Promise<any[]> {
  return callAKShare("stock_lhb_ggtj_dtl_em", {
    start_date: date,
    end_date: date,
  });
}
...
export async function getStockInfo(symbol: string): Promise<StockInfo | null> {
  try {
    const data = await callAKShare<any[]>("stock_individual_info_em", {
      symbol,
    });
...
export async function getStockHistory(
  ...
  try {
    ...
    const startDate = formatDateYYYYMMDD(
      new Date(Date.now() - count * 2 * 24 * 60 * 60 * 1000)
    );
    ...
    console.error(
      `[AKShare] getStockHistory(${symbol}, ${period}) failed:`,
      error?.message
    );
    return [];
  }
...
export async function getStockMinuteHistory(
  ...
  try {
    ...
    console.error(
      `[AKShare] getStockMinuteHistory(${symbol}, ${period}) failed:`,
      error?.message
    );
    return [];
  }
 ...
 */
export async function getStockFundFlow(
  symbol: string,
  market: "sh" | "sz" | "bj" = "sz"
): Promise<FundFlowAK[]> {
  try {
...
/**
 * 获取单个股票的资金流排名
 * @param stockCode 股票代码（如 "300433"）
 */
export interface FundFlowRankInfo {
  symbol: string;
  rank: number; // 资金流排名
  mainNetInflow: number; // 主力净流入
  mainNetInflowPercent: number; // 主力净流入占比
...
export async function getFundFlowRankBySymbol(
  stockCode: string
): Promise<FundFlowRankInfo | null> {
  try {
    ...
    return {
      ...
      mainNetInflow: parseFloat(item["今日主力净流入-净额"]) || 0,
      mainNetInflowPercent: parseFloat(item["今日主力净流入-净占比"]) || 0,
      totalCount: allRanks.length,
    };
  } catch (error: any) {
    console.error(
      `[AKShare] getFundFlowRankBySymbol(${stockCode}) failed:`,
      error?.message
    );
    return null;
  }
 ...
 * @returns 返回排名、排名变化等信息
 */
export interface StockHotRankInfo {
  symbol: string;
  rank: number; // 当前排名
...
export async function getHotRankLatestBySymbolEM(
  symbol: string
): Promise<StockHotRankInfo | null> {
  try {
    ...
    return {
      symbol: formattedSymbol,
      rank: parseInt(dataMap.rank) || 0,
      rankChange: parseInt(dataMap.rankChange) || 0,
      marketAllCount: parseInt(dataMap.marketAllCount) || 5000,
      calcTime: dataMap.calcTime || "",
    };
  } catch (error: any) {
    console.error(
      `[AKShare] getHotRankLatestBySymbolEM(${symbol}) failed:`,
      error?.message
    );
    return null; // 返回null让前端能正常显示降级数据
  }
}
 ...
 */
export async function getXueqiuHotRank(): Promise<any[]> {
  try {
    return await callAKShare(
      "stock_hot_follow_xq",
      { symbol: "最热门" },
      10000
    ); // 10秒超时
  } catch (error: any) {
    console.error("[AKShare] getXueqiuHotRank failed:", error?.message);
    return [];
  }
}
 ...
 * @param stockCode 股票代码（如 "300433"）
 */
export interface XueqiuRankInfo {
  symbol: string;
  rank: number; // 在雪球的排名
  followers: number; // 关注数
}

export async function getXueqiuRankBySymbol(
  stockCode: string
): Promise<XueqiuRankInfo | null> {
  try {
    const allRanks = await getXueqiuHotRank();
    if (!allRanks || allRanks.length === 0) return null;

    // 雪球返回的代码格式是 SZ300433 或 SH600519
    const shCode = `SH${stockCode}`;
    const szCode = `SZ${stockCode}`;

    const idx = allRanks.findIndex((item: any) => {
      const code = item["股票代码"] || "";
      return code === shCode || code === szCode || code === stockCode;
    });
    ...
    console.error(
      `[AKShare] getXueqiuRankBySymbol(${stockCode}) failed:`,
      error?.message
    );
    return null;
  }
 ...
 */
export async function getNorthFlowIn(
  indicator: "north" | "south" = "north"
): Promise<any[]> {
  if (indicator === "north") {
 ...
 */
export async function getNorthHoldStock(
  market: "沪股通" | "深股通" = "沪股通"
): Promise<any[]> {
  return callAKShare("stock_hsgt_hold_stock_em", {
    market,
    indicator: "今日排行",
  });
}
  ...
  try {
    ...
    if (Array.isArray(spotData) && spotData.length > 0) {
      ...
      for (const stock of spotData) {
        const raw =
          stock["涨跌幅"] ?? stock["changePercent"] ?? stock["pct_chg"] ?? 0;
      ...
      console.log(
        `✅ Real market breadth: ↑${riseCount} ↓${fallCount} →${flatCount} | 涨停${limitUpCount} 跌停${limitDownCount} (total: ${spotData.length})`
      );
